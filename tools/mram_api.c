...
int Send(char *device_name, int macro_row, int macro_col, int ip_addr, int mram_addr, unsigned int *host_data_addr, int data_size)
{
    ...
    rc = Store(device_name, 0, cmd_buffer, pos);

    // ring controller to process cmd
    ring_ctrl(CTRL_REG_START_WRITE, IAR, pos);
    IAR = IAR + pos * 4 > RES_DATA_START_OFFSET ? 0 : IAR + pos * 4;
...
int Recv(char *send_device_name, char *recv_device_name, int macro_row, int macro_col, int ip_addr, int mram_addr, unsigned int *host_data_addr, int data_size)
{
    ...
    // Trans command
    rc = Store(send_device_name, 0, cmd_buffer, pos);
    ...
    ring_ctrl(CTRL_REG_START_READ, RES_DATA_START_OFFSET, data_size * 2);

    // wait to recv data
    fd = open(recv_device_name, O_RDWR);
    rc = read_to_buffer(recv_device_name, fd, (char *)host_data_addr, data_size * 4, RES_DATA_START_OFFSET);
out:
    free(cmd_buffer);
    close(fd);
    return rc;
...
int Vmmul(char *device_name, unsigned int *input_vector, int input_size, int inbits, int in_group, int macro_row, int macro_col, int mram_addr, int wbits, int device_ram_addr)
{
    ...
    // ring controller to read res data
    ring_ctrl(CTRL_REG_START_READ, RES_DATA_START_OFFSET, in_group * actived_macro_size * 7);
