 ...
 * @param device_ram_addr the RAM address on the device.
 ...
 * int weight[4][512] = {{28, 11,..., 24, 31},{15, 19,... 2, 26}, {30, 17,..., 11, 9}, {3, 7,..., 11, 8}};
 ...
 * int weight_in_bits[4][5][16] = {...};
 * Send("/dev/xdma0_h2c_0", 0, 1, 0, 0, weights, 0, (int *)(weight_in_bits[0]), 5 * 16);
 * Send("/dev/xdma0_h2c_0", 0, 1, 1, 0, weights, 0, (int *)(weight_in_bits[1]), 5 * 16);
 * Send("/dev/xdma0_h2c_0", 0, 1, 2, 0, weights, 0, (int *)(weight_in_bits[2]), 5 * 16);
 * Send("/dev/xdma0_h2c_0", 0, 1, 3, 0, weights, 0, (int *)(weight_in_bits[3]), 5 * 16);
 *
 * @param macro_row the macro array shape is 4x4, row number range is [0, 3]
 ...
 * int weight_in_bits[5][16];
 * Send("/dev/xdma0_h2c_0", "/dev/xdma0_c2h_0", 0, 1, 0, 0, (int *)(weight_in_bits), 5 * 16);
 ...
 * int vector[28] = {1,2,...28};
 * int weight[4][28] = {{28, 11,...24, 31},{15, 19,...2, 26}, {30, 17,...11, 9}, {3, 7,...11, 8}};
 *
 * // fill the original data to 512
 * int vector_filled[512] = {0, 0, 0, 0,...0, 0, 1, 2,...28};
 * int weight_filled[4][512] = {{0, 0, 0,...28, 11,...24, 31},{0, 0, 0,...15, 19,...2, 26}, {0, 0, 0, 0,...30, 17,...11, 9}, {0, 0, 0,,.3, 7,...11, 8}};
 *
 * // Omitting the bit slicing process. Assume that vector and weight matrix has a maximum length of 5 bits,
 * // After bitwise slicing process, the shape of weight matrix is [col][bitsize][16].
 * // Here bitsize is 5. Since a macro unit stores 32 bits of data, each col has 512 bits data in total, the final length is 16.
 * int vector_filledin_bits[5][16] = {...};
 * int weight_filled_in_bits[4][5][16] = {...};
 * // Store weight to macro01, offset 0x0008
 * Send("/dev/xdma0_h2c_0", 0, 1, 0, 0, weights, 0x0008, (int *)(weight_filled_in_bits[0]), 5 * 16);
 * Send("/dev/xdma0_h2c_0", 0, 1, 1, 0, weights, 0x0008, (int *)(weight_filled_in_bits[1]), 5 * 16);
 * Send("/dev/xdma0_h2c_0", 0, 1, 2, 0, weights, 0x0008, (int *)(weight_filled_in_bits[2]), 5 * 16);
 * Send("/dev/xdma0_h2c_0", 0, 1, 3, 0, weights, 0x0008, (int *)(weight_filled_in_bits[3]), 5 * 16);
 *
 * // send calculation command
 * int res_addr = 0x1000;
 * Vmmul("/dev/xdma0_h2c_0", (int *)vector_filledin_bits, 5 * 16, 5, 1, 0, 1, 0x0008, 5, res_addr)
 *
 * // read cal result
 * int calc_result[4];
 ...
 * int ram_addr = 0x0008;
 * int ram_vector_in_bits[5][16] = {...};
 * Store("/dev/xdma0_h2c_0", ram_addr, (int *)ram_vector_in_bits, 5 * 16);
 *
 * //You need to pre-load the weight vector to Macro the method then called is basically the same as Vmmul
 * int ip_addr = 0;
 * int mram_addr = 0x0012;
 * int macro_vector_in_bits[5][16] = {...};
 * Send("/dev/xdma0_h2c_0", 0, 1, ip_addr, mram_addr, (int *)(macro_vector_in_bits), 5 * 16);
 *
